  ...
  return (
    <>
      ...
      <TabPanel id={part.code}>
        <div className="flex flex-wrap gap-[7px] pt-4">
          {part.sections.map((s) => (
            <Chip
              key={s.code}
        ...
        <section className="pb-7 pt-5">
          ...
          {section.descriptionVi && (
            <p className="mb-4 max-w-[70ch] text-[13.5px] text-fg2">{section.descriptionVi}</p>
          )}
              ...
              <ListRow
                ...
                trailing={
                  <span className="flex items-center gap-2.5">
                    <span className="rounded-[5px] border border-transparent bg-(image:--g-soft) px-1.5 py-0.5 text-[10.5px] font-semibold text-acc-hi">
                      {s.level.replace('_PLUS', '+')}
                    </span>
                    <span
                      className={cn(
                        'w-11 text-right text-[12.5px] font-semibold tabular-nums',
