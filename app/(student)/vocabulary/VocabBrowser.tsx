...
export function VocabBrowser({
  ...
  return (
    <>
      <div className="flex flex-wrap gap-[7px] pb-6">
        {topics.map((t) => (
      ...
      {topic && (
        <div className="mb-4 flex items-baseline gap-3 border-t border-ln pt-6">
          <h2 className="text-[15px] font-semibold">{topic.nameVi}</h2>
        ...
        {entries.map((v) => (
          <li key={v.id} className="border-b border-ln py-4 first:border-t">
            <div className="flex flex-wrap items-baseline gap-3">
              <span className="jp text-[19px] font-bold">{v.headword}</span>
              <span className="jp text-[13px] text-fg2">{v.readingKana}</span>
              <span className="text-[11.5px] text-fg3">{v.meaningVi}</span>
            ...
            {v.noteVi && <p className="mt-2 max-w-[75ch] text-[12.5px] text-fg2">{v.noteVi}</p>}
          </li>
        ))}
      </ul>
    </>
