...
import { AUDIO_FALLBACK_WAVEFORM } from '@/constants/common/audio';
...
export function AudioPlayer({
  ...
  const peaks = waveform?.length ? waveform : AUDIO_FALLBACK_WAVEFORM;
  ...
  return (
    <div className={cn('flex items-center gap-3.5 rounded-lg border border-ln px-4 py-3.5', className)}>
      ...
      <div aria-hidden className="flex h-6 flex-1 items-center gap-0.5">
        {peaks.map((h, i) => (
          <i
            ...
            className={cn(
              'min-h-0.75 flex-1 rounded-sm transition-colors duration-150',
      ...
      {playOnce && (
        <span className="flex-none text-xs text-fg3">
