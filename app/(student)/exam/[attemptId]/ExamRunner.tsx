...
export function ExamRunner({ attemptId, title, playOnce, parts, mediaUrls, exitHref }: ExamRunnerProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      toast('Mất kết nối. Kiểm tra mạng rồi bấm Nộp bài lại.', 'ng');
    ...
    if (body.code === 'ALREADY_SUBMITTED') {
      // Đã nộp rồi (ví dụ bấm ở tab khác) — xem kết quả là đúng việc cần làm.
      router.push(`/result/${attemptId}`);
      return;
    }
    if (body.code === 'TIME_EXCEEDED') {
      toast('Đã quá thời gian làm bài, lượt này không được chấm.', 'wr');
      router.push(exitHref);
      return;
    }
    toast('Không nộp được bài. Thử lại giúp tôi nhé.', 'ng');
    submittedRef.current = false;
    setSubmitting(false);
  }
    ...
    <div className="fixed inset-0 z-[60] flex flex-col bg-bg">
      <div className="relative flex h-[50px] flex-none items-center gap-4 border-b border-ln px-5">
        ...
        {multiPart ? (
            ...
            {parts.map((p, i) => {
              ...
              return (
                <li
                  ...
                  className={cn(
                    'jp flex items-center gap-1.5 text-[13px]',
          ...
          <span className="jp gt flex-none text-[13px] font-bold">{part.nameJa}</span>
        )}
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-y-auto p-6">
          ...
          {spokenOptions ? (
            <div role="radiogroup" aria-label="Phương án trả lời">
              <p className="mb-3 text-[12.5px] text-fg3">
                ...
                {question.options.map((o) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'grid aspect-square place-items-center rounded-lg border text-[22px] font-semibold tabular-nums',
      ...
      <Modal open={confirmEnd} onClose={() => setConfirmEnd(false)} title={`Kết thúc phần ${part.order}`}>
        <div className="p-6">
          <p className="jp gt mb-1 text-[20px] font-bold">{part.nameJa}</p>
          <p className="mb-2 text-[15px] font-medium">Kết thúc phần {part.order}?</p>
          <p className="mb-6 text-[13px] text-fg2">
          ...
          <div className="flex justify-end gap-2.5">
            <button
              ...
              className="rounded-lg border border-ln px-4 py-2.5 text-[13.5px] transition-colors duration-200 hover:border-acc-dim hover:bg-ln2"
            >
              Ở lại
            </button>
            <button
              type="button"
              onClick={finishPart}
              className="rounded-lg bg-(image:--g) px-4 py-2.5 text-[13.5px] font-semibold text-on-g transition-[filter] duration-200 hover:brightness-110"
            >
