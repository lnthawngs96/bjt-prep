...
export function ScoreRuler({ score, className }: { score: number; className?: string }) {
  ...
  return (
    <div className={cn('relative pt-[22px]', className)}>
      ...
      <span
        aria-hidden
        style={{ left: `${pct}%` }}
        className="absolute top-[15px] h-[21px] w-0.5 -translate-x-1/2 rounded-sm bg-fg"
      />
      <span
        style={{ left: `${pct}%` }}
        className="gt absolute -top-0.5 -translate-x-1/2 text-[11.5px] font-bold tabular-nums"
      >
        {score}
      </span>

      <div className="mt-2.5 flex text-[10.5px] text-fg3">
        {SCORING_BANDS.map((b) => {
          const isCurrent = b.level === current.level;
