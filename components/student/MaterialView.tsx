...
export function MaterialView({
  ...
  if (material.kind === 'IMAGE') {
    if (!mediaUrl) return null;
    return (
      <figure className="mb-5 overflow-hidden rounded-[10px] border border-ln">
        <Image
          src={mediaUrl}
  ...
  if (material.kind === 'TABLE') {
    const body = material.body as TableBody | null;
    if (!body) return null;
    const numeric = new Set(body.numericColumns ?? []);
    return (
      <div className="mb-5 rounded-[10px] border border-ln px-5 py-4">
        {body.caption && (
          <div className="jp mb-3 border-b border-ln pb-2 text-[11.5px] text-fg3">{body.caption}</div>
        )}
            ...
            <thead>
              <tr>
                {body.headers.map((h) => (
                  <th
                    key={h}
                    className="jp border border-ln bg-(image:--g-soft) px-2.5 py-[7px] text-xs font-semibold"
                  >
                    {h}
                  </th>
                ))}
              ...
              {body.rows.map((row, i) => (
                <tr key={i}>
                  {row.map((cell, j) => (
                    <td
                      key={j}
                      className={`border border-ln px-2.5 py-[7px] ${
  ...
  if (material.kind === 'CHART') {
    const body = material.body as ChartBody | null;
    if (!body) return null;
    const max = Math.max(...body.series.flatMap((s) => s.values), 1);
    return (
      <div className="mb-5 rounded-[10px] border border-ln px-5 py-4">
        {body.caption && (
          <div className="jp mb-4 border-b border-ln pb-2 text-[11.5px] text-fg3">{body.caption}</div>
        )}
        <div className="flex h-40 items-end gap-4">
          {body.categories.map((cat, ci) => (
            <div key={cat} className="flex flex-1 flex-col items-center gap-1.5">
              ...
              <span className="jp text-[10.5px] text-fg3">{cat}</span>
            </div>
          ))}
        </div>
        <div className="mt-3 flex flex-wrap gap-4 border-t border-ln pt-3 text-[11.5px] text-fg2">
          {body.series.map((s, si) => (
