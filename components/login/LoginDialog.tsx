  ...
  return (
    <Modal open={open} onClose={hide} title="Đăng nhập để bắt đầu">
      <div className="relative px-8 pt-9 text-center">
        ...
        <div className="relative mx-auto mb-5 grid size-12 place-items-center rounded-xl bg-(image:--g) text-xl font-bold text-on-g shadow-btn-lg">
          B
        </div>
        <p className="relative mb-2 text-xl font-bold tracking-tight">
          Đăng nhập để bắt đầu
        </p>
        <p className="relative mx-auto max-w-3xs text-sm leading-relaxed text-fg2">
      ...
      <div className="px-8 pt-6">
        ...
        {error && (
          <p role="alert" className="mt-3 text-center text-xs text-ng">
            {error}
          </p>
        )}

        {!googleConfigured && (
          <p className="mt-3 rounded-lg border border-wr/40 bg-wr-soft px-3 py-2.5 text-xs leading-relaxed text-wr">
      ...
      <ul className="mb-1.5 flex flex-col gap-3 px-8">
        {LOGIN_PERKS.map(([before, strong, after]) => (
          <li key={strong} className="flex items-start gap-3 text-xs leading-relaxed text-fg2">
            <span className="mt-px grid size-4 flex-none place-items-center rounded-sm bg-(image:--g-soft)">
      ...
      <div className="mt-6 border-t border-ln px-8 pb-7 pt-6 text-center text-xs leading-relaxed text-fg3">
...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden className="size-5 flex-none">
      <path
        fill="#4285F4"
